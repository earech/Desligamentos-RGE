...
def consultar(page) -> list:
    ...
    print(f"Pesquisando desligamentos em {MUNICIPIO} de {formatar_data(hoje)} até {formatar_data(fim)} ...")
    ...
    # Fallback: texto geral da página
...
def montar_mensagem(desligamentos: list) -> tuple:
    ...
    if not desligamentos:
        corpo = (
            ...
            f"Período verificado: {periodo}\n\n"
            "---\nMonitor automático RGE via GitHub Actions"
        )
        return False, corpo

    linhas = [
        f"⚠️ ALERTA: {len(desligamentos)} desligamento(s) encontrado(s) em {MUNICIPIO}!",
        f"Período verificado: {periodo}",
        "",
    ]
...
def main():
    with sync_playwright() as p:
        ...
        try:
            ...
            print(f"❌ Timeout ao carregar ou interagir com o site: {e}")
            browser.close()
            sys.exit(1)
    ...
    print(corpo)
    exportar_github_output(tem_alerta, corpo)
    sys.exit(0)
